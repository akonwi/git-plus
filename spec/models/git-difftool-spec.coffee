...
describe "GitDiffTool", ->
  ...
  describe "when git-plus.includeStagedDiff config is true", ->
    it "calls git.cmd with 'diff-index HEAD -z' and optional '-d' for directories", ->
      expect(git.cmd).toHaveBeenCalledWith ['diff-index', 'HEAD', '-z'], cwd: repo.getWorkingDirectory()
      expect(git.cmd).toHaveBeenCalledWith ['diff-index', 'HEAD', '-z', '-d'], cwd: repo.getWorkingDirectory()
