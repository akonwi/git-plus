...
import GitStageFilesBeta from './models/git-stage-files-beta'
import GitStageHunk from './models/git-stage-hunk'
import ManageStash from './models/manage-stash'
...
import GitUnstageFiles from './models/git-unstage-files'
    ...
    if (repos.length > 0) {
      ...
      this.subscriptions.add(
        atom.commands.add('atom-workspace', {
          ...
          'git-plus:manage-stash': ManageStash,
      ...
      if (atom.config.get('git-plus.experimental.stageFilesBeta'))
        this.subscriptions.add(
          atom.commands.add('atom-workspace', 'git-plus:stage-files', () =>
            git.getRepo().then(GitStageFilesBeta)
          )
        )
      else
        this.subscriptions.add(
          atom.commands.add('atom-workspace', {
            'git-plus:unstage-files': () => git.getRepo().then(GitUnstageFiles),
            'git-plus:stage-files': () => git.getRepo().then(GitStageFiles)
          })
        )
      ...
      this.subscriptions.add(
        ...
        atom.config.observe('git-plus.experimental.autoFetch', interval =>
          this.autoFetch(interval)
        ),
        atom.config.onDidChange('git-plus.experimental.stageFilesBeta', () => {
