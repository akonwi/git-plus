...
import { gitAdd, gitAddModified } from "./models/add";
import GitPaletteView from "./views/git-palette-view";
import * as treeViewCommands from "./models/tree-view";
import { gitChangeToNewBranch } from "./models/git-checkout-new-branch";
...
import { showLog, LogViewURI } from "./models/git-log";
    ...
    if (repos.length > 0) {
      ...
      this.subscriptions.add(
        atom.commands.add("atom-workspace", {
          ...
          "git-plus:add": () => gitAdd.invoke(),
          "git-plus:add-all": () => gitAdd.invoke({ stageEverything: true }),
          "git-plus:add-modified": () => gitAddModified.invoke(),
          "git-plus:commit": () => {
          ...
          "git-plus:new-branch": () => gitChangeToNewBranch.invoke(),
          ...
          "git-plus:remove": () => gitRemove.invoke({ showSelector: true }),
          "git-plus:remove-current-file": () => gitRemove.invoke(),
          "git-plus:reset": () => gitReset(),
          "git-plus:show": () => git.getRepo().then(repo => GitShow(repo)),
          "git-plus:log": () => showLog.invoke(),
          "git-plus:log-current-file": () => showLog.invoke({ onlyCurrentFile: true }),
