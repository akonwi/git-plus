...
class GitRevisionView
  ...
  @showRevision: (editor, revHash, options={}) ->
    console.log('revision view options', options)
    ...
    stdout = (output) ->
      fileContents += output
      console.log('revision output', output)
    exit = (code) ->
      console.log('revision file content', fileContents)
      if code is 0
    ...
    showArgs = ["show", "#{revHash}:./#{path.basename(file)}"]
    console.log("calling git", showArgs, options.cwd)
