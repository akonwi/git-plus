...
module.exports =
  ...
  activate: (state) ->
    ...
    @subscriptions.add atom.commands.add 'atom-workspace', 'git-plus:commit-all-and-push', -> git.getRepo().then((repo) -> git.add(repo).then -> GitCommit(repo, stageChanges: true, andPush: true))
